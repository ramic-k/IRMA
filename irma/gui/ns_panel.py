...
class _CoeffFields:
    ...
    * any non-blank    -> blanks read as ``0.0``; trailing blanks are dropped so
    ...
    def __init__(self, parent):
        ...
        self.c1 = LabeledEntry(parent, "c1  x E (per meV/meV):", default="",
...
HELP = {
    "phonopy_yaml": (
        ...
        "with FORCE_CONSTANTS/FORCE_SETS), NOT a primitive-cell mesh.yaml dump -- "
        "a mesh.yaml will fail with a 'FORCE_CONSTANTS inconsistent / p2s_map' "
        "error.\n\nThis defines the lattice, atom positions and force constants "
        "from which S(Q,E) (and the Debye-Waller factors) are computed.\n\n"
    ...
    "force_constants": (
        ...
        "blank = read the force constants embedded in phonopy.yaml, or "
        "auto-discover a sibling FORCE_CONSTANTS/FORCE_SETS next to it. Only set "
        "this if your constants live in a separate file the yaml does not point to."),
    "force_sets": (
    ...
    "mesh": (
        ...
        "mesh -> smoother S(Q,E) and Debye-Waller, more cost (cost ~ nx*ny*nz). "
        "20x20x20 is fine for a survey; 40x40x40 (the default) for production. "
        "Use the same density on each axis unless the cell is very anisotropic."),
    "temperature": (
        "Sample temperature in Kelvin.\n\nSets the Bose mode populations (how much "
        "up-scattering / multiphonon) and the Debye-Waller attenuation (high-Q "
        "fall-off). Default 296 K (room temperature). Use the actual experiment "
        "temperature -- e.g. 5 K for a cryogenic VISION run."),
    "scatterers": (
        "One line per DISTINCT element in your phonopy.yaml:\n"
        "    SYMBOL,sigma_bound_b,awr[,b_coh_fm[,sigma_inc_b]]\n"
        "(comma-separated, no spaces). Fields:\n\n"
        "  SYMBOL         element symbol; must match the atoms in phonopy.yaml.\n"
        "  sigma_bound_b  bound scattering cross section [barn]\n"
        "                 (~ sigma_coh + sigma_inc). Normalizes S(alpha,beta).  REQUIRED.\n"
        "  awr            atomic weight ratio A = M / m_n (nuclide mass / neutron\n"
        "                 mass). Sets the recoil + Debye-Waller mass.  REQUIRED.\n"
        "  b_coh_fm       coherent scattering length [fm]; CAN BE NEGATIVE (H = -3.74).\n"
        "                 Drives the COHERENT elastic Bragg comb. OPTIONAL -- omit\n"
        "                 it and that element contributes no coherent-elastic line.\n"
        "  sigma_inc_b    incoherent (bound) cross section [barn]. Drives the\n"
        "                 INCOHERENT elastic Debye-Waller line. OPTIONAL -- omit it\n"
        "                 and there is no incoherent elastic for that element.\n\n"
        "IMPORTANT: the FIRST line is the PRINCIPAL scatterer -- its sigma_bound_b "
        "and awr set the overall S(alpha,beta) normalization and recoil mass, so "
        "put the dominant/representative element first.\n\n"
        "Get the numbers from a standard neutron table (NIST / Sears 1992), for "
        "the natural element or a specific isotope.\n\n"
        "The default row is natural carbon from IRMA's built-in nuclear "
        "table (Rauch-Waschkowski/Sears). Typing a symbol (Be, O, ...) and "
        "leaving the cell autofills the still-blank columns from the same "
        "table; values you typed yourself are never overwritten. Nuclides "
        "with energy-dependent scattering lengths (B, Cd, Gd, ...) are not "
        "prefilled -- enter constants for your energy range by hand.\n\n"
        "MODE 0 (DOS) extras -- append as key=value tokens on the same line:\n"
        "  dos=<file>   2-column phonon DOS for this element (when DOS source=file)\n"
        "  unit=<u>     its frequency unit: meV (default), eV, cm-1 or THz\n"
        "  mult=<n>     number of atoms of this element in the cell (inelastic weight)\n"
        "  pos=x:y:z;.. fractional sites, ';'-separated (coherent-elastic comb)\n"
        "  e.g.  H,80.27,0.999,-3.74,80.26,dos=h.txt,mult=2,pos=0:0:0;0.5:0.5:0.5"),
    "inelastic_mode": (
        "Fidelity of the inelastic (phonon) calculation, for the Phonopy-model "
        "input.\n\n"
        "  1 = incoherent approximation (default): fast, phonon-DOS-like; the "
        "standard choice for most materials and surveys.\n"
        "  2 = coherent one-phonon + incoherent-approx multiphonon: exact "
        "one-phonon dispersion with an incoherent-approximation multiphonon tail. "
        "Needed for strong coherent scatterers (graphite, Be, ...) where the "
        "dispersion structure matters; several times slower.\n"
        "  0 = DOS + isotropic Debye-Waller: the lightweight incoherent-"
        "approximation phonon expansion, with the partial DOS derived from this "
        "phonopy calc (no eigenvector engine). Cheapest; per-atom normalized like "
        "1/2. (To feed your OWN DOS files instead, switch 'phonon input' to "
        "'DOS files'.)"),
    "lattice": (
        "Unit cell for the inelastic-mode-0 COHERENT-elastic Bragg comb (the "
        "iel=10 analogue): a,b,c,alpha,beta,gamma (Angstrom, degrees), comma- or "
        "space-separated. Used only by mode 0 (modes 1/2 read the cell from "
        "phonopy.yaml). Pair it with each element's b_coh_fm + the per-element "
        "fractional 'positions' column."),
    "input_source": (
        "Where the phonon information comes from.\n\n"
        ...
        "  DOS files (mode 0): you provide a 2-column phonon DOS file per "
        "element -- no phonopy needed. This is the lightweight DOS path for "
        "H-rich / incoherent / disordered materials, or when you only have a DOS."),
    "autofill": (
    ...
    "elements": (
        ...
        "                 the coherent-elastic Bragg comb. Optional.\n"
        "  sigma_inc_b    incoherent bound xs [barn]; drives the incoherent\n"
        "                 elastic Debye-Waller line. Optional.\n"
        "  mult           (DOS-files mode 0) atoms of this element in the cell.\n"
        "  unit           (DOS-files mode 0) frequency unit of the DOS file.\n"
        "  DOS file       (DOS-files mode 0) the 2-column phonon DOS for this\n"
        "                 element.\n"
    ...
    "dos_format": (
        "DOS file: 2 columns -- frequency, DOS intensity. Whitespace OR comma "
        "separated. Lines starting with '#' (and non-numeric header lines) are "
        "skipped. Intensity units are arbitrary (renormalized internally). The "
        "frequency unit of column 1 is chosen per element (the 'unit' column)."),
    "max_phonon_order": (
    ...
    "n_directions": (
        ...
        "More directions -> smoother spectrum, less orientational sampling noise, "
        "more cost (roughly linear). Default 10000 is well converged for "
        "production runs (the value the validation campaign used, and the "
        "NCrystal exporter default); drop to ~1000 for a quick look."),
    "mp_directions": (
        ...
        "~50-100). The multiphonon part is smooth, so it needs far fewer "
        "directions than the one-phonon term."),
    "jobs": (
    ...
    "elastic": (
        "Whether to add an elastic line at zero energy transfer.\n\n"
        "  on (default): include the elastic line (see 'elastic kind').\n"
        "  off: inelastic spectrum only (no E=0 peak)."),
    "elastic_kind": (
        "Which elastic channels to include (when elastic line = on).\n\n"
        "  both (default): coherent Bragg comb + incoherent Debye-Waller line -- "
        "the physically complete elastic for most crystals (both channels are "
        "present in real materials).\n"
        "  coherent: Bragg comb only (needs b_coh_fm in the scatterers).\n"
        "  incoherent: Debye-Waller line only (needs sigma_inc_b) -- e.g. for "
        "amorphous or purely-incoherent scatterers with no Bragg peaks."),
    "incoherent_elastic_mode": (
        ...
        "The coherent Bragg comb is unaffected (already directional per "
        "reflection). The ENDF tape path cannot represent this option."),
    "include_gain": (
        "Add the energy-GAIN (anti-Stokes, E < 0) side.\n\n"
    ...
    "gain_side": (
        "How the energy-GAIN (E < 0) side is computed when 'include energy-gain' "
        "is on.\n\n"
        "'direct (explicit Bose factors)': evaluate the gain side from first "
        "principles with explicit Bose phonon-annihilation factors -- the more "
        "accurate default; gain and loss are computed independently.\n\n"
    ...
    "e_min": (
        "Lowest energy transfer in the output spectrum [meV].\n\nDefault 0.0 "
        "(energy-loss side only). Use a NEGATIVE value (e.g. -50) to include the "
        "energy-gain side in the output window (requires 'include energy-gain')."),
    "e_max": (
        "Highest energy transfer in the output spectrum [meV].\n\nMust cover the "
        "phonon band plus its multiphonon tail. Default 250 meV suits most "
        "molecular/lattice modes; raise it for hard modes (e.g. C-H stretches ~400)."),
    "de": (
    ...
    "dq": (
        "Momentum-transfer support spacing [1/A] for the underlying S(Q,E).\n\n"
        "Smaller -> finer Q sampling along the instrument locus, more cost. "
        "Default 0.05 1/A. This is the ONE meaning of dQ (the S(Q,E) shell "
        "spacing); it is not the per-shell sub-sampling."),
    "q_max": (
        "Optional hard cap on the Q-support [1/A].\n\nDefault: blank = derive the "
        "Q range from the instrument locus over the chosen energy range. Set a "
        "value only to truncate it."),
    "ind_ef": (
        "Fixed final energy Ef [meV] for an indirect-geometry instrument (the "
        "analyzer selects Ef; the incident energy is scanned).\n\nDefault 3.5 meV, "
        "which -- together with the 45/135 deg banks below -- reproduces the "
        "VISION spectrometer. Change Ef and the angles for a different indirect "
        "instrument."),
    "ind_angles": (
        "Detector bank centre angles 2-theta [deg].\n\nEither a comma list "
        "'30,60,90,120,150' or a range 'start:stop:step' (e.g. '30:150:30'). One "
        "spectrum is computed per bank (each bank keeps its own curve).\n\nDefault "
        "'45,135' = the VISION forward/backward banks."),
    "dir_ei": (
        "Fixed INCIDENT energy Ei [meV] for a direct-geometry instrument (a "
        "chopper selects Ei; the final energy is measured).\n\nDefault 300 meV, "
        "above the default E max of 250.\n\nNote: E max must be < Ei -- the "
        "neutron cannot lose more than its incident energy."),
    "dir_angles": (
        "Detector bank 2-theta [deg]: a comma list '10,60,120' or a range "
        "'start:stop:step' (e.g. '5:135:5')."),
    "q_cuts": (
        "Optional constant-|Q| cuts: a comma list of Q values [1/A] (e.g. "
        "'2.0,4.0,6.0'). For each Q you get I(E) at that FIXED |Q| -- a vertical "
        "slice through S(Q,E), which reads off the modes living at that Q.\n\n"
        "This is a DIRECT-geometry product (hence it lives on the Direct tab): a "
        "direct instrument spans a broad Q-E region, so a fixed-|Q| slice is "
    ...
    "output_mode": (
        ...
        "  2-D map: the dense 2-D S(Q,E) heat-map over a Q x E grid, shown in the "
        "Plot tab (and exportable there). Direct-geometry only.\n\n"
    ...
    "cut_by": (
        ...
        "detector group).\n"
        "  constant-Q: vertical slices of S(Q,E) at fixed |Q| values you list, "
        "each integrated over a band of width 'cut dQ'.\n\n"
        "For an arbitrary angle- or Q-integration not covered here, compute the "
        "2-D map and post-process it yourself."),
    "cut_dq": (
        "Half-/bin width [1/A] over which each constant-Q cut is AVERAGED -- a "
        "real measurement integrates a finite |Q| band, not an infinitely-thin "
        "line. The cut at Q0 averages S(Q,E) over |Q| in [Q0 - dQ, Q0 + dQ].\n\n"
        "Default: blank = an infinitely-thin interpolated slice at exactly Q0."),
    "map_coverage": (
        "Detector angular coverage 2theta (min,max in degrees) the 2-D map masks "
        "to -- the instrument's accessible-(q,E) 'arch'. Enter as 'min,max' "
        "(e.g. '3,135').\n\n"
    ...
    "map_mask": (
        ...
        "above -- i.e. show only the kinematically accessible region (the "
        "Euphonic-style arch) rather than the full computed S(Q,E) surface with "
        "the envelope drawn over it.\n\n"
    ...
    "resolution_shape": (
        "Instrument energy-resolution line shape, applied by convolving the "
        "spectrum with a kernel of energy-dependent width.\n\n"
    ...
    "sigma_c2": (
        ...
        "(0.00000081)."),
    "resolution_model": (
        "How the DIRECT-geometry energy resolution is determined.\n\n"
        "  width polynomial: use the shared 'resolution width c0,c1,c2' below "
        "(a Gaussian sigma poly) -- the simple generic option.\n"
    ...
    "chop_frequency": (
        ...
        "CNCS/LET. Higher frequency -> sharper resolution but lower flux. Fermi "
        "machines run ~120-600 Hz; the disk machines ~60-300 Hz. The resolution "
        "narrows and the burst time shortens as frequency rises."),
    "output": (
    ...
    "export_components": (
        ...
        "  ON: each cut additionally carries its inelastic and elastic channels, "
        "written to the file (extra columns/arrays) and overlaid on the plot.\n\n"
...
class NSPanel(ttk.Frame):
    ...
    def _build_material(self, parent):
        ...
        self.inelastic_mode = LabeledCombobox(
            ...
            default=self._MODE_LABELS[0], help_text=HELP["inelastic_mode"])
    ...
    def _build_physics(self, parent):
        ...
        self._check_with_help(g, "include energy-gain side", self.include_gain,
                              HELP["include_gain"])
    ...
    def _sync_ns_context(self):
        ...
        # The mode-0 COHERENT comb needs the crystal (lattice + per-element
        ...
        # mode-0 elastic line is requested so the comb is one field away.
    ...
    def _build_direct_output(self, parent):
        ...
        self.dir_output = LabeledCombobox(
            ...
            default="fixed cuts", help_text=HELP["output_mode"])
    ...
    def _build_output(self, parent):
        ...
        ttk.Checkbutton(ctl, text="mask to accessible (q,E)  [2-D map]",
                        variable=self.map_mask, command=self._replot).pack(
    ...
    def _on_geometry_tab(self, _evt=None):
        """Geometry tab changed: refresh the map-button state."""
        self._sync_map_btn()
