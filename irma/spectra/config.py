...
class Scatterer:
    ...
    Bragg comb together with ``material.lattice`` and ``b_coh_fm``.
    """
    symbol: str
...
class MaterialConfig:
    ...
    Bragg comb (the iel=10 analogue); it is unused by modes 1/2.
    """
...
class PhysicsConfig:
    ...
    inelastic_mode: Union[int, str] = 1     # 0/'dos' | 1/'incoherent' | 2/'coherent' (1ph + incoherent-approx multiph)
...
def validate(cfg: SpectraConfig) -> SpectraConfig:
    ...
    if p.inelastic_mode == 0:
        ...
        # fractional sites + b_coh_fm) enables the Bragg comb. Validate it only
        ...
        if m.lattice is not None:
            ...
            if not all(0.0 < ang < 180.0 for ang in (al_, be_, ga_)):
                raise SpectraConfigError(
                    ...
                    f"{(al_, be_, ga_)}")
            # positions + b_coh_fm are required only when the coherent comb is
...
def _elastic_inputs(m, p):
    ...
    (``run_map``) so ``physics.elastic`` means the same thing in both:
      ...
      * ``inelastic_mode == 0``-> the DOS-derived tape-free line, built inside
        ...
        Bragg comb when material.lattice + positions + b_coh_fm are given, the
...
def run_map(cfg, *, q_min=0.5, q_max=None, dQ_map=0.1, angle_range=None,
    ...
    Config-driven defaults (a None argument defers to the config):
      * ``q_max``      -> ``grid.q_max_invA`` (or 12.0 if that is unset);
    ...
    if q_max is None:
        q_max = g.q_max_invA if g.q_max_invA else 12.0
    if q_max <= q_min:
        raise SpectraConfigError(
            f"run_map q_max ({q_max}) must exceed q_min ({q_min}) -- q_max "
            "defaults to grid.q_max_invA (else 12.0) when not passed")
