...
Coherent elastic  (THERMR ``sigcoh``; ENDF-102 LTHR=1)
    ...
    powder differential cross section is therefore a comb of Bragg peaks
...
class ElasticModel:
    ...
    def coherent_dsigma_dOmega(self, Q, q_res=0.0):
        ...
        the bare comb (zero except exactly on a peak -> use q_res for spectra).
        """
...
def from_endf_mf7mt2(path, T_K=296.0, label=None):
    ...
    handling per channel: the coherent comb selects the tabulated
    ...
    if LTHR in (1, 3):
        ...
        if abs(Tsel - T_K) > _T_MATCH_TOL_K:
            ...
            warnings.warn(
                ...
                f"{Tsel:g} K Bragg comb.", stacklevel=2)
    if LTHR in (2, 3):
...
def instrument_reach_emax_eV(*, q_max_invA, e_fixed_meV=None, q_cuts=None,
    ...
    forward model can evaluate, so the tape-free comb builders stop enumerating
    ...
    The elastic line is only ever evaluated at
      ...
      * the constant-Q cuts at Q0 +/- cut_dq, where the Bragg comb is broadened
    ...
    if qs:
        ...
        tail = 40.0 * max(float(q_res_invA), 0.0)      # Gaussian-comb underflow reach
...
# inelastic kernel by construction. The coherent comb reuses the byte-pinned
# irma.core.elastic_dw kernels (the same arithmetic the MF7/MT2 writer uses), so
# it equals from_endf_mf7mt2 applied to an elastic_mode=2 (MEF) tape up to NJOY
...
def _finalize_bragg_comb(E_edge_meV, f_bragg):
...
def from_engine_elastic_state(elastic_state, *, b_coh_fm, sigma_inc_b, awr,
    ...
    elastic_kind : {"both", "coherent", "incoherent"}
        ...
        coherent Bragg comb AND a separate incoherent Debye-Waller line. The
        single-channel modes isolate one: ``"coherent"`` -> Bragg comb only,
    ...
    principal_species : int, optional
        ...
        ``|b_coh|``); selects the coherent comb's isotropic Debye-Waller
        reference. The incoherent line sums EVERY species' channel and does
    ...
    incoherent_elastic_mode : {"isotropic", "directional"}
        ...
        exporter's ``incoherent_elastic_mode``. The coherent comb is
        unaffected (it is already directional per reflection).
    ...
    # coherent Bragg comb (scale=1.0 -- no ENDF-SEF sigma_inc fold) PLUS a
    ...
    # explicit rather than folded into the comb.
    ...
    if want_coh:
        ...
        # entirely for the incoherent-only line, which never uses the comb.
    ...
    E_edge_meV, f_bragg, Q_bragg = _finalize_bragg_comb(E_edge_meV, f_bragg)
...
# omitting the directional F-matrices; for an isotropic crystal the comb matches
...
def from_dos_and_lattice(crystal, *, awr, sigma_inc_b, f0_lambda, T_K,
    ...
    inelastic_mode 1/2: the coherent Bragg comb keeps its native per-atom
    ...
    T_K : float
    elastic_kind : {"both", "coherent", "incoherent"}
        ``"both"`` -> coherent Bragg comb AND the summed incoherent DW line; the
    ...
    if want_coh:
        ...
        if nedge > 0:
            ...
            species_dw = resolve_species_dw(crystal_info, tempr, 1)
            # scale=1.0: keep the comb's native PER-ATOM normalization (the 1/N in
    ...
    # weight, matching the per-atom coherent comb and inelastic.
...
# Bragg comb over [2th_min, 2th_max] gives the clean closed form
...
def bank_elastic_area(model: ElasticModel, E_fixed, two_theta_deg,
    ...
    dtheta_deg : half-width of the bank's 2theta acceptance (>0 integrates the
                 Bragg comb robustly; 0 falls back to the point differential)
    ...
    """
    if dtheta_deg <= 0.0:
        # point differential at the bank centre (smooth incoherent + bare comb)
...
# -----------------------------------------------------------------------------
# Self-test: the comb integrates back to sigma_coh(E)=S(E)/E
# -----------------------------------------------------------------------------
def selftest(model: ElasticModel, E_meV=200.0, q_res=0.0):
    ...
    Qmax = 2.0 * k
    if q_res <= 0.0:
        # use the analytic comb sum directly
