...
def _get_engine_context(*, phonopy_yaml, force_constants, force_sets,
    ...
    context, key = get_or_build_context(
        ...
        num_jobs=int(n_jobs),
        sigma_mev=0.0, multiphonon_max_order=int(multiphonon_max_order),
...
def _build_mode0_elastic_model(*, dos_species, m0, dos_crystal, elastic_kind,
    ...
    Bragg comb. Returns ``None`` (with a NOTE) when the requested channel
...
def compute_spectrum(*, geometry, phonopy_yaml, temperature_k, mesh,
    ...
    ``dos_species`` entry: the coherent Bragg comb + incoherent DW line are built
    ...
    ``"both"`` (default: coherent Bragg comb + incoherent DW line), or one of
    ...
    # elastic comb is only ever evaluated at the banks' elastic-Q windows
    ...
    if dos_species is not None:
        ...
        # mode-0 elastic line: coherent Bragg comb + incoherent DW from the
        # DOS-derived isotropic Debye-Waller coefficient f0 (contin). The
        # coherent comb needs a user-supplied crystal (dos_crystal =
...
def compute_sqe_map(*, geometry, phonopy_yaml, temperature_k, mesh, sab_mass_ratio,
    ...
    # map's Q_max through the q_cuts tail gives the comb the full 40-sigma
    # Gaussian underflow headroom at the grid edge, so the q_res-broadened comb
    # on Q_grid is bit-identical to a full enumeration.
    if elastic_model is None and elastic:
        from irma.spectra.elastic import instrument_reach_emax_eV
        elastic_emax_eV = instrument_reach_emax_eV(
