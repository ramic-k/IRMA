...
def parse_scatterer(s):
    ...
    ``pos=x1:y1:z1;x2:y2:z2`` (fractional sites for the coherent-elastic comb).
...
def _add_common(p):
    ...
    p.add_argument("--inelastic-mode", type=int, choices=[0, 1, 2], default=1,
                   help="0 = DOS + isotropic Debye-Waller (no eigenvectors), "
                        "1 = incoherent approximation, 2 = exact coherent "
                        "one-phonon + incoherent multiphonon (default: 1)")
    ...
    p.add_argument("--lattice", type=parse_coeffs, metavar="a,b,c,al,be,ga",
                   help="mode 0 only -- unit cell [Angstrom, degrees] for the "
                        "coherent-elastic Bragg comb (with per-scatterer pos= "
                        "sites)")
...
def build_parser():
    ...
    mapp.add_argument("--q-min", type=float, default=0.5)
    mapp.add_argument("--q-max", type=float, default=None,
                      help="map Q maximum [1/A]; default = config grid.q_max_invA "
                           "(or 12.0 if unset)")
    mapp.add_argument("--dq-map", type=float, default=0.1, help="map Q step [1/A]")
