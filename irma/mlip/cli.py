...
def _add_emit_options(p):
    ...
    p.add_argument("--nuclide", action="append", metavar="SYM=LABEL",
                   help="explicit nuclide identity, e.g. H=2-H (repeatable; "
                        "default: most abundant isotope, printed)")
    ...
    p.add_argument("--principal", default=None,
                   help="spectra principal scatterer [first species]")
...
def _build_parser():
    ...
    ec.add_argument("--dry-run", action="store_true",
                    help="print the provisioning commands and exit")
...
def _do_emit(bundle, targets, args) -> int:
    ...
    if "spectra" in targets:
        produced.append(emit_spectra_yaml(
            ...
            principal=args.principal, nuclides=nuclides,
            overrides=overrides,
...
def _cmd_env(args) -> int:
    ...
    if args.env_command == "create":
        ...
        print(f"  provisioning a dedicated environment for "
              f"{args.potential} ...")
