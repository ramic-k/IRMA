...
def _batched_qpoints_eigh(dynamical_matrix, qpoints):
    ...
    ``sqrt(|eigval|) * sign(eigval) * VaspToTHz`` — with ONE stacked
    ...
    try:
        # phonopy >= 2.48; phonopy.units.* is deprecated and slated for
        # removal. Both expose the same constant, so results are unchanged.
        from phonopy.physical_units import get_physical_units
        vasp_to_thz = get_physical_units().DefaultToTHz
    except ImportError:
        from phonopy.units import VaspToTHz as vasp_to_thz

    dynmat = run_dynamical_matrix_solver_c(dynamical_matrix, qpoints, None)
    eigvals, eigvecs = np.linalg.eigh(dynmat)
    eigvals = eigvals.real
    frequencies = np.sqrt(np.abs(eigvals)) * np.sign(eigvals) * vasp_to_thz
