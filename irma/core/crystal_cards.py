...
def _parse_extinction_card(reader, elastic_mode):
    ...
    reader.require(
        ...
        f"extinction {model}: no active channel — set l>0 (primary) or "
...
def _parse_crystal_cards(reader, za, nphon, ncold=0, nsk=0, nss=0, b7=0.0):
    ...
    for iat in range(nat):
        ...
        reader.require(at_A >= 1, f"A must be >= 1, got {at_A}")
    ...
    if ext_cfg is not None:
        # Extinction attenuates the coherent Bragg comb, so only the
