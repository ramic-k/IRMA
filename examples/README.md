...
| # | Calculation type | Example | Runtime (16-core laptop) |
|---|------------------|---------|---------|
| 1 | Classic LEAPR deck, built-in coherent elastic (`iel=1`) | `tsl/README.md` §2 → committed graphite deck under `tests/` | seconds |
| 2 | Incoherent + free-gas secondary scatterer (`iel=0`, `nss=1`) | `tsl/README.md` §2 → committed H-in-CH₂ deck | ~1 min |
...
| 4 | Liquid: translation + oscillators + cold H₂ | `tsl/README.md` §2 → committed l-CH₄ and ortho-/para-H₂ decks | seconds (l-CH₄) to ~40 s (H₂) |
| 5 | Two-pass (bound) secondary scatterer (`b7=0`) | `tsl/README.md` §2 → committed BeO deck | seconds |
| 6 | **Generalized elastic, any crystal** (`iel=10`, `inelastic_mode=0`) | `tsl/graphite_iel10_classic.input` | ~10 s |
| 7 | Bragg-edge grouping (ENDF-102 §7.2.2) | `tsl/README.md` §1 (one-line variant of #6) | ~10 s |
| 8 | Phonopy-backed law (`iel=10`, `inelastic_mode=1/2`) | `tsl/graphite_mode2.input` (production: auto-sized multiphonon) | 1–2 min |
| 9 | **Crystalline extinction** (`iel=10` + optional `extinction` card) | `tsl/be_iel10_extinction.input` | ~15 s |
| 10 | **INS spectrum from a DOS file** (spectra mode 0) | `spectra/graphite_mode0_dosfile.yaml` | seconds |
| 11 | INS spectrum, DOS derived from phonopy (mode 0) | `spectra/graphite_mode0_phonopy.yaml` | seconds |
| 12 | INS spectrum, full eigenvector engine (modes 1/2, VISION) | `spectra/graphite_mode2_vision.yaml` | seconds |
| 13 | Direct-geometry 2-D S(Q,E) map, chopper resolution (ARCS) | `spectra/graphite_arcs_map.yaml` | seconds |
| 14 | Direct-geometry constant-Q cuts | `spectra/graphite_direct_qcuts.yaml` | seconds |
