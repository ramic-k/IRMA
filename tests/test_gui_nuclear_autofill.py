...
irma.core.nuclear_data instead of hardcoded literals:
  ...
  2. ElementTable.nuclear_defaults -- the NS / NCrystal panels' default carbon
     row is built from the table, so the panels track the compilation.
...
def test_phonopy_autofill_preserves_provenance_for_later_symbol_edit(
    ...
    # the default row IS the machine-filled carbon row from panel startup
    assert panel.element_table.rows[0]["_var"]["symbol"].get() == "C"
    panel._autofill_from_phonopy()
