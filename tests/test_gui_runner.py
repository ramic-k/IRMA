...
# ---- S8: cancel/spawn races + shutdown reaping -------------------------------
def _delayed_popen(monkeypatch):
    """Monkeypatch the runner's Popen with one that blocks until released,
    holding the spawn window (run started, _proc still None) open so a cancel
    can be issued deterministically inside it. Returns (entered, release,
    spawned): `entered` fires once the worker is inside Popen, `release`
    lets the real spawn proceed, `spawned['proc']` is the real Popen object."""
    ...
    monkeypatch.setattr(runner_mod.subprocess, "Popen", slow_popen)
    return entered, release, spawned


@pytest.mark.skipif(os.name != "posix", reason="process-group kill is POSIX")
def test_cancel_during_spawn_window_is_not_dropped(monkeypatch):
    """S8 race 1: cancel() between run_command and the worker's Popen used to
    return without recording intent (_proc was None) -- the cancel was lost
    and the run completed 'successfully'. It must kill the child and report
    the run cancelled."""
    entered, release, spawned = _delayed_popen(monkeypatch)
    r, logs, done, ev = _make_runner()
    r.run_command([sys.executable, "-u", "-c", "import time; time.sleep(60)"],
                  success_msg="should never be reported")
    assert entered.wait(10), "worker never reached Popen"
    r.cancel()          # _proc is still None: the old code silently dropped this
    release.set()
    assert ev.wait(20), "done_callback never fired"
    assert done["ok"] is False and "cancelled" in done["msg"].lower()
    assert "proc" in spawned, "child was never spawned"
    assert _wait(lambda: spawned["proc"].poll() is not None, timeout=15), \
        "child survived a spawn-window cancel"
    assert _wait(lambda: not r.is_running, 10)


@pytest.mark.skipif(os.name != "posix", reason="process-group kill is POSIX")
def test_immediate_cancel_on_second_run_kills_new_child(monkeypatch):
    """S8 race 2: _proc was never cleared when a run finished, so an immediate
    cancel on a SECOND run signaled the previous (exited) process -- the new
    child survived while the GUI reported 'Calculation cancelled.'. The cancel
    must target and kill the NEW run's child."""
    # First run: completes normally (leaves a stale _proc in the old code).
    r, logs, done, ev = _make_runner()
    r.run_command([sys.executable, "-u", "-c", "print('first')"],
                  success_msg="first ok")
    assert ev.wait(20) and done["msg"] == "first ok"
    assert _wait(lambda: not r.is_running, 10)

    # Second run: hold its spawn window open and cancel inside it.
    entered, release, spawned = _delayed_popen(monkeypatch)
    done2, ev2 = {}, threading.Event()

    def on_done2(ok, msg):
        done2["ok"], done2["msg"] = ok, msg
        ev2.set()

    r.run_command([sys.executable, "-u", "-c", "import time; time.sleep(60)"],
                  success_msg="second ok", on_done=on_done2)
    assert entered.wait(10), "worker never reached Popen"
    r.cancel()          # old code: signaled the exited first proc, new child lived
    release.set()
    assert ev2.wait(20), "second run's done_callback never fired"
    assert done2["ok"] is False and "cancelled" in done2["msg"].lower()
    assert "proc" in spawned, "second child was never spawned"
    assert _wait(lambda: spawned["proc"].poll() is not None, timeout=15), \
        "second run's child survived the cancel"
    assert _wait(lambda: not r.is_running, 10)
