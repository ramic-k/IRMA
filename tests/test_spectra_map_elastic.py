...
tests pin the new contract:
  ...
  * all three config routes reach the map: mode-0 DOS-derived (lattice-free
    incoherent + crystal Bragg comb), an explicit ElasticModel (the ENDF-tape
...
def test_map_mode0_coherent_elastic_bragg_comb():
    """dos_crystal enables the Bragg comb: edges appear in the metadata and the
    deposited area matches the q_res=dQ_map-broadened reference comb + DW line."""
    ...
    assert np.allclose(got, area, rtol=1e-9)
    # the comb gives the line genuine Q structure (Bragg peaks), unlike the
