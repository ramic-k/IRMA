...
# A minimal two-edge Bragg comb. The CEF/SEF incoherent builder ignores
# bragg/nedge entirely, but the MEF builder writes a coherent comb too and
# rejects an empty one, so the MEF fixtures pass these.
...
def _write_iel10_tape(path, za, awr, crystal_info, npr=1, bragg=None):
    ...
    there; the MEF builder needs a real comb (see _BRAGG)."""
...
def _parsed_mt2(path):
    ...
    For LTHR=3 the coherent comb lands in T0/LT/S_T0_table (+ per-LT 'T'/'S')
...
def test_mef_coherent_block_npr_invariant(mef_npr2_tape, mef_npr1_tape):
    """Only the incoherent SB carries npr. The coherent comb (T0 TAB1 plus
...
def test_mef_reader_coherent_comb_matches_tape(mef_npr2_tape):
    ...
    follows): the de-cumulated Bragg comb reproduces the tape's cumulative
