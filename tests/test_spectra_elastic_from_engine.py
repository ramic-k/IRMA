...
reproduced exactly from the lattice, the coherent comb integrates back to
...
Combs are enumerated to emax_eV=0.3 where the assertion is reach-independent
...
def test_selftest_holds_for_coherent_comb():
    m = from_engine_elastic_state(_state(), b_coh_fm=6.646, sigma_inc_b=0.001,
                                  awr=11.898, elastic_kind="coherent",
                                  emax_eV=0.3)
...
def test_both_carries_coherent_and_incoherent_channels():
    """Default 'both' = pure Bragg comb + separate incoherent DW (no fold)."""
    ...
    # the coherent comb is PURE (no (sigma_coh+sigma_inc)/sigma_coh fold): its
    # f_bragg matches the coherent-only build exactly, regardless of sigma_inc.
