...
Pure-physics checks encoding the verified conventions/derivations: elastic comb
normalization, incoherent Debye-Waller form, detailed balance, indirect/direct
kinematics, S(alpha,beta) loader conventions, the bank-integrated elastic cross-
check, and parser robustness on published tapes.

CI COVERAGE: most of these need NO external data -- they run on every checkout.
The elastic parser + comb/edge/bank self-consistency gates run against a real
...
# the elastic parser + comb/edge/bank gates run on CI with no external tree. The
...
def test_elastic_comb_integrates_to_sigma_coh(graphite_endf):
    """Comb normalization self-test on the in-repo published graphite tape: the
    Bragg-summed elastic cross section equals sigma_coherent at every energy."""
...
def test_elastic_comb_generalizes_to_beryllium():
...
def test_parser_handles_published_endfb_graphite(graphite_endf):
    ...
    the comb self-test; first Bragg edge near 1.876 1/A. Runs on CI."""
    m = el.from_endf_mf7mt2(graphite_endf, T_K=T)
