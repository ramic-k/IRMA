...
# ---- extinction path -------------------------------------------------------------
def _two_edge_comb(F_sites, uniform):
    """(200)+(400) comb with 3-tuple dir terms and a matching sdw."""
    planes = [_plane(_IN_PHASE, hkl=(2, 0, 0)),
              _plane(_IN_PHASE, hkl=(4, 0, 0))]
    bragg, dir_terms = [], []
...
def test_extinction_site_resolved_delta_matches_reference():
    ...
    bragg, dir_terms, sdw_ne = _two_edge_comb(F_ne, uniform=False)
    ...
    bragg_u, dir_terms_u, sdw_eq = _two_edge_comb(F_eq, uniform=True)
