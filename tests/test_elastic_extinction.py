...
def test_no_extinction_limit_matches_kinematic():
    # l=g=L=0 -> y == 1 everywhere -> sigma equals the plain kinematic comb
    sdw = _synthetic_species_dw()
    bragg = _synthetic_bragg()[0]
...
def test_extinction_s_table_is_histogram_and_monotonic():
    ...
    # minimal kinematic comb (one node per edge energy, cumulative kinematic S)
    Ek, Scum, acc = [], [], 0.0
    for bj in bragg:
        acc += bj[1]
...
def test_extinction_composes_with_grouped_high_e_comb():
    # Extinction reuses kin_table's above-cutoff nodes verbatim, so feeding it a
    # GROUPED comb (sparse high-E) yields fewer high-E nodes than a plain comb,
    ...
    # splice point `top` fixed (as the real Be comb does); grouping only collapses
    # the >1 eV nodes.
    mid = [0.2, 0.5, 0.8]
...
def test_extinction_splice_consumes_real_grouped_comb():
    """#12: feed the extinction splice the comb produced by the REAL grouping
...
def _weak_comb_plus_strong(n_weak=25, planes_per_weak=21, fsq_weak=1e-12,
                           e_strong=0.01, fsq_strong=5.0):
    """The adversarial comb from the 2026-07-09 pre-release review: a long run
...
def test_late_strong_reflection_is_not_spliced_kinematically():
    ...
    sdw = _synthetic_species_dw()
    bragg, dir_terms = _weak_comb_plus_strong()
    ...
    # returned sigma must reflect that, not the kinematic comb.
    assert ext < 0.05 * kin
...
def test_scan_continues_past_soft_cap_until_tail_is_proven():
    ...
    sdw = _synthetic_species_dw()
    bragg, dir_terms = _weak_comb_plus_strong(planes_per_weak=2100)  # 52.5k
    cfg = _cfg(model="BC_pure", l=1.0e7, g=0.0, L=0.0)
    sigma_fn, _, E_active = make_sigma_coh_ext(
        bragg, dir_terms, sdw, [0.0], _V, _N, 1.0, cfg, [296.0])
    assert E_active > 0.01
...
def test_scan_hard_cap_raises_instead_of_splicing(monkeypatch):
    ...
    sdw = _synthetic_species_dw()
    bragg, dir_terms = _weak_comb_plus_strong(planes_per_weak=50)   # 1,251
