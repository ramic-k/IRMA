...
The tape-free elastic builders must not enumerate the Bragg comb to the ENDF
...
driver keeps its own full 5 eV call and is untouched. Pinned here:
  ...
  * the helper's reach algebra (bank elastic ring, Q-support, cut bands,
    5 eV cap);
  * truncation purity: a smaller-emax comb is the bit-identical PREFIX of a
    larger-emax comb, for BOTH builders;
...
def test_helper_extends_for_q_cuts_with_gaussian_tail_headroom():
    """A constant-Q cut at Q0 +/- band evaluates the q_res-broadened comb; the
...
def test_committed_example_spectrum_identical_to_full_5ev_enumeration(monkeypatch):
    ...
    constant-Q cuts to exercise the q_res-broadened comb path): forcing the old
