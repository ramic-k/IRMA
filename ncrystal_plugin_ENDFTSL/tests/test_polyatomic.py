...
comb replicated on both tapes must, after fraction-weighting, sum back to the single
per-atom comb (this is exactly the ENDF/B-VIII.1 BeO layout, where both tapes carry
the full per-atom comb).
"""
import math
import os
...
def test_fraction_weighting_recovers_per_atom_all_channels():
    ...
    assert single.coh_cumS, "fixture tape must carry a coherent comb"
    ...
    for pk in packs:
        ...
        # coherent comb is fraction-weighted
