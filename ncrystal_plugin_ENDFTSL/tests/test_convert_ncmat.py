...
def test_structure_free_ncmat_has_no_cell():
    s = ncmat.structure_free_ncmat("C", 2.26, "graphite.endftslpack")
...
def test_ncmat_rejects_nonpositive_or_nonfinite_density():
    for bad in (0.0, -2.26, float("nan"), float("inf")):
        with pytest.raises(ValueError, match="density_g_cm3"):
            ncmat.structure_free_ncmat("C", bad, "graphite.endftslpack")
