...
def test_inelastic_grids_physical_and_lat_unscaled():
    ...
    # downscatter table is rectangular [alpha][beta] and non-negative
    assert len(law.sab_asym_downscatter) == len(law.alpha_phys)
    assert all(v >= 0.0 for row in law.sab_asym_downscatter for v in row)
