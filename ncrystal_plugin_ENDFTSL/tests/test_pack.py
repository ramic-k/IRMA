...
_MALFORMED = [
    ...
    ("meta.note", {"metadata": {"note": "a\nb"}}),              # newline injection
    ("meta.note", {"metadata": {"note": "a\tb"}}),              # control char
    ("metadata key", {"metadata": {"bad key": "v"}}),           # whitespace in key
    ("metadata key", {"metadata": {"a=b": "v"}}),               # '=' abuse in key
]
...
_TAMPERED = [
    ...
    ("meta.note", {"meta.note": "meta.note = a\x01b"}),         # control char
]
...
# --- post-fix review PACK-1 sub-findings ------------------------------------

def test_orphan_elastic_msd_is_a_clean_error(tmp_path):
    """elastic_msd_a2 without its partner used to crash the writer with a raw
    TypeError (float(None))."""
    p = _good_pack(elastic_incoherent_xs_barn=None)
    with pytest.raises(ValueError, match="all-or-none"):
        write_pack(p, tmp_path / "x.endftslpack")


def test_orphan_elastic_xs_is_rejected_not_dropped(tmp_path):
    """elastic_incoherent_xs_barn without an MSD used to be SILENTLY DROPPED
    on write (the block is keyed on the MSD field)."""
    p = _good_pack(elastic_msd_a2=None)
    with pytest.raises(ValueError, match="all-or-none"):
        write_pack(p, tmp_path / "x.endftslpack")


@pytest.mark.parametrize("bad_id", [
    "gra\nphite",                 # newline: injects a raw key=value line
    "gra\nsab_values = 0 0 0 0",  # targeted field injection
    "", 5, "gra\x00phite",
])
def test_material_id_injection_is_rejected(tmp_path, bad_id):
    """material_id is written verbatim into the line-oriented container; a
    newline inside it becomes an arbitrary pack field on re-read (the
    metadata VALUES were already guarded; the id was missed)."""
    p = _good_pack(material_id=bad_id)
    with pytest.raises(ValueError, match="material_id"):
        write_pack(p, tmp_path / "x.endftslpack")


def test_material_id_with_equals_is_rejected(tmp_path):
    p = _good_pack(material_id="a = b")
    with pytest.raises(ValueError, match="material_id"):
        write_pack(p, tmp_path / "x.endftslpack")
