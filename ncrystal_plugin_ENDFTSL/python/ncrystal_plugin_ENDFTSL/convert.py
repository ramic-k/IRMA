...
def build_pack(ev: TSLEvaluation, T: float, material_id: str,
    ...
    #   - coherent comb: * coherent_scale. The whole-crystal comb is a per-atom
    #     quantity, but tapes store it under different conventions (per-atom
...
class SpeciesSpec:
    ...
    symbol: str
    mass: float
    fraction: float | None = None  # atom fraction; weights this tape's coherent comb
...
def build_packs(specs, T: float, material_id: str,
    ...
    Coherent elastic is the whole-crystal per-atom Bragg comb, but tapes store it
    differently and the convention is NOT always recoverable from one tape:
      - per-atom comb replicated on >=2 coherent tapes (standard ENDF/B-VIII.1,
        IRMA MEF): scale each by f -> sum_i f_i*sigma = sigma_coh. Unambiguous.
      - sole coherent carrier (the other species are LTHR=2): a single LTHR=1 comb
    ...
    if n_coh >= 2:
        ...
        # tapes carry the SAME per-atom whole-crystal comb AND their atom fractions
        # total 1 (so sum_i f_i*comb == comb). Otherwise the weight-1.0 C++ sum yields
        ...
        if abs(coh_frac_sum - 1.0) > 1e-6:
            raise ValueError(
                ...
                f"to {coh_frac_sum:.6g} != 1; a per-atom whole-crystal Bragg comb cannot "
                "be reconstructed by fraction-weighting a partial set (standard ENDF "
                "replicates the per-atom comb on every principal tape).")
        ...
        # comb with different edge ENERGIES (ENDF rounding ~1%; e.g. SiO2-alpha Si vs O
        ...
        # and is robust to edge jitter, while a genuinely PARTITIONED per-species comb
        ...
        for c in coh_data:
            ...
            if median > 0.10:
                raise ValueError(
                    f"coherent-bearing tapes carry DIFFERENT Bragg combs (median "
                    f"sigma_coh deviation {median:.2f}); fraction-weighting assumes the "
                    "per-atom whole-crystal comb is replicated across principal tapes. A "
    ...
    for sp, ev, hc in zip(specs, evs, has_coh):
        f = float(sp.fraction)
        if not hc:
            coh_scale = 1.0                      # no comb on this tape
        elif n_coh >= 2 or f >= 1.0 - 1e-12:
            coh_scale = f                        # replicated per-atom (or monatomic)
        elif coherent_convention == "cef_scaled":
            coh_scale = f                        # 1/f_DC comb -> x f = per-atom
        elif coherent_convention == "per_atom":
            coh_scale = 1.0                      # per-atom comb -> take as-is
        else:
            raise ValueError(
                f"{sp.symbol!r} is the SOLE coherent-elastic carrier (atom fraction "
                f"{f}); a single LTHR=1 comb is ambiguous between a per-atom comb "
                "(standard ENDF, scale 1) and a 1/f_DC-scaled comb (IRMA SEF, scale "
