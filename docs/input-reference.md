...
This page is the authoritative, card-by-card reference for the IRMA input
deck. An IRMA deck is written in NJOY free-format and follows the LEAPR card
structure, so if you have driven LEAPR before the layout will feel familiar.
The cards are read in the fixed order given below; conditional cards (the
generalized-elastic block, the per-temperature detail block, the phonopy
controls) are clearly flagged with the mode that triggers them. Two complete,
runnable decks (a classic mode-0 deck and a generalized `iel=10`,
`inelastic_mode=2` deck) close out the page so you can copy, adapt, and run.

!!! note "Running a deck"
    ```bash
    python -m irma <input_file> <output_file>
    ```
    `<input_file>` is the LEAPR-style deck; `<output_file>` is the path where
    the ENDF-6 MF7/MT4 tape is written. The `irma` console script and the GUI
    (`irma-gui` or `python -m irma --gui`) accept the same decks.

## Deck format rules

IRMA tokenizes input exactly the way NJOY's list-directed (free-format) reader
does. Knowing these few rules prevents the great majority of deck problems.
...
| **Integer vs. float fields** | Integer-coded fields reject non-integral values: `200.0` is accepted as `200`, but `1.9` is a deck error rather than a silent truncation. |
...
| **Comments** | Only text after the card's `/` on the same line is ignored (any marker works there: `-- …`, `$ …`, plain text). There is **no standalone comment-line syntax**: a line without a `/` is parsed as data: mid-deck it mis-feeds the next card read, and in the trailing comment section it silently becomes an MF1/MT451 comment card. |

!!! note "Job streams are tolerated"
    A full NJOY job stream is accepted on input. IRMA locates the
    `leapr … stop` block automatically and ignores the surrounding modules,
    so you can point IRMA at an existing NJOY deck without editing it.

!!! warning "Error reporting names the card and the line"
    When a deck is malformed, IRMA raises a `DeckError` that names the card
    being read, the expected/found values, and the **input line number** in
...
deck. The generalized-elastic block (Cards 6b–6g) is read only when `iel=10`,
between Card 6 and Card 7. The per-temperature detail block (Cards 11–19) is
read only for the classic paths and `inelastic_mode=0`.
...
| **4** | `mat za isabt ilog smin [iint]` | `0 0 0 0 1.0e-75 0` | ENDF `MAT` (`≥ 1`) and `za`; `isabt` (0/1) selects S(α,−β) storage; `ilog` (0/1) selects log storage; `smin` is the minimum stored S; optional `iint` (0/1) selects the MF7/MT4 interpolation law (see below, default 0). **For `iel=10`, `za` must encode the physical nuclide as `1000·Z + A`** so the principal scatterer can be matched to a Card 6d atom. |
| **5** | `awr spr npr iel ncold nsk` | `0 0 0 0 0 0` | Principal scatterer: mass ratio `awr` (`> 0`), free-atom cross section `spr` (`> 0`), atom count `npr` (`≥ 1`), elastic option `iel`, cold-hydrogen option `ncold` (0–4), pair-correlation option `nsk` (0–2). See the `iel` table below. |
...
!!! warning "Set `ilog=1` (Card 4) for cryogenic temperatures"
    With `ilog=0` the symmetric law is stored linearly as `S·exp(−β/2)`. Below
    ~50–100 K, `β = E/kT` is large enough that high-energy-transfer values
    underflow to zero in the ENDF field, silently dropping the optic-mode
    structure on read-back. Set `ilog=1` (ENDF `LLN=1`, log storage) for
    low-temperature tapes; IRMA warns at write time if you leave it at 0 and
    lose data. See
    [Troubleshooting](troubleshooting.md#high-energy-optic-phonon-structure-missing-at-low-temperature).

!!! tip "`iint=1` (Card 4) for coherent one-phonon laws"
    `iint` is the sibling of `ilog`: `ilog` chooses how S is *stored*, `iint`
    chooses how S is *interpolated* between stored points, the ENDF `INT`
    flag written on both the α (per-β `TAB1`) and β (`TAB2`) tables of the
    MF7/MT4 law.

    - `iint=0` (default): log-linear (`INT=4`) — classic, NJOY-faithful.
    - `iint=1`: linear-linear (`INT=2`). Coherent one-phonon laws have
      structural near-zeros (e.g. the graphite (002) dip) that logarithmic
      interpolation floors, biasing the cross section **low** in the thermal
      minimum. lin-lin preserves them; an `INT`-aware THERMR honors whichever
      flag is written.

    Use `iint=1` **only with a Q/α grid converged near sharp coherent peaks**:
    lin-lin still chords across a peak the grid does not resolve. The default
    is byte-identical to pre-`iint` output.
...
| `10` | Generalized coherent elastic: Bragg edges computed from the user's crystal structure (any material). Cards 6b–6g follow. Recommended path for new evaluations. |
...
|-------|---------|
| `0` | None. |
| `1` | Vineyard. Cards 17–19 must be supplied, but, matching NJOY LEAPR, **no correction is applied**: with `ncold = 0` the resulting tape is identical to `nsk = 0` (with `ncold > 0`, the `S(κ)` table is used by the cold-hydrogen kernel exactly as it would be for any `nsk`). Use `nsk = 2` for a correction that modifies the law. |
...
!!! warning "`inelastic_mode=1/2` restrictions"
    The phonopy-backed noncubic modes (`inelastic_mode=1` or `2`, set on
    Card 6b) do not support cold hydrogen/deuterium, the Sköld/Vineyard
    pair-correlation option, or a secondary scatterer. Decks combining
...
| Field | Default | Notes |
|-------|---------|-------|
| `elastic_mode` | — | `1` = SEF (single-channel elastic format): one elastic channel per tape. Single atom type: the dominant channel carries the full elastic strength: coherent (LTHR=1) if `σ_coh > σ_inc`, else incoherent (LTHR=2). Polyatomic: the designated-coherent atom gets LTHR=1, every other principal gets LTHR=2 (see [Theory](theory.md#elastic-format-sef-vs-mef-card-6b-field-1)). `2` = MEF (Mixed Elastic Format): LTHR=3 (coherent + incoherent together). Must be 1 or 2. |
| `nat` | — | Number of distinct atom types in the unit cell (`≥ 1`). |
| `nspec` | — | Number of partial phonon spectra that follow (Card 6e). Meaningful only for `inelastic_mode=0`. **For `inelastic_mode=1/2`, `nspec` must be 0** and Card 6e is omitted (deck error otherwise). |
...
(`alpha = b^c`, `beta = a^c`, `gamma = a^b`), each in `(0, 180)`. An angle
...
| `npos` | Number of positions of this atom in the unit cell (`≥ 1`). |
...
!!! note
    ...
    the phonopy model. A mismatch (wrong species, wrong positions, or a
    principal ZA that has no matching phonopy site) is reported as a Card 6d
    deck error, not as a phonopy failure.

!!! note "Principal nuclide split over several Card 6d entries (`inelastic_mode=1/2`)"
    For `inelastic_mode=1/2`, multiple Card 6d atom types that carry the
    **principal** nuclide (the Card 4 `za`) are merged into a single group:
    their position lists are combined, and the MT4 law accumulates over every
    represented site. The merged entries must agree in `awr`, `b_coh`, and
    `sigma_inc`; differing values are a Card 6d deck error. Non-principal
...
!!! warning
    ...
    omitted (deck error otherwise).

!!! note "Each partial spectrum must carry its species' full vibrational weight"
    The per-species Debye-Waller integral normalizes each Card 6e spectrum
    with a continuous weight of 1 (the `tbeta = 1` convention): there is no
    ...
    **all** of its species' vibrational weight; a spectrum carrying only part
    of it would bias that species' Debye-Waller lambda.

!!! note "Atom types without a Card 6e spectrum inherit the principal's Debye-Waller lambda"
    ...
    spectrum). For the principal type itself this is exact. For any **other**
    type it is only an approximation, and it directly sets that species'
    elastic `W'(T)` on the tape, so IRMA prints a warning naming the type
    (and flags the inherited lambda as likely poor when the two masses differ
    by more than 20%). To give such a species its own lambda, raise `nspec`
    on Card 6b and supply a Card 6e spectrum for it.
...
| Field | Notes |
|-------|-------|
| phonopy.yaml path | Quoted path to the phonopy model. Force constants are read from the YAML if embedded, otherwise discovered **next to it** in the order `force_constants.hdf5`, `FORCE_CONSTANTS`, `FORCE_SETS`. If none is found this is a hard error; files in the process working directory are never consulted. |
...
average. (Decks written before v0.16 carried a 4-field layout whose 3rd field
selected a removed diagnostic method; drop that field: `10000 1000 0 1 /`
becomes `10000 1000 1 /`.)

!!! note "Give the law genuine support"
    Transfers beyond the tabulated law are covered downstream by THERMR's
    short-collision-time extension (driven by the tape's effective
    temperature). Make sure `nphon` (or `auto_order=1`) gives the law real
    support across the requested beta grid. The auto-sizer raises the order to
    ...
    truncates the high-Q rows (the engine warns).

#### Extinction card — crystalline extinction (optional)

An optional card placed **last in the `iel=10` block** (after Cards 6d/6e, or
Card 6g for `inelastic_mode=1/2`), before Card 7. Off by default; reduces the
coherent-elastic Bragg comb for dynamical-diffraction extinction (a *sample*
property). Needs a coherent-elastic channel: `elastic_mode=2` (MEF) always has
one; with `elastic_mode=1` (SEF) the deck must actually route MF7/MT2 to the
coherent builder, otherwise the card is rejected at parse time (see
[Crystalline extinction](extinction.md) for the two SEF configurations this
excludes).
...
!!! note "Automatic grids"
    The GUI (and the bundled grid generators) can build these grids for you.
    The beta grid has three regions: a log lower tail, a linear phonon region
    over `(0, freq_max)`, and a log upper tail (default `beta_max = 5 eV`). The
    alpha grid is **linear in momentum transfer Q**: points every `dq`
    (default 0.05 Å⁻¹) up to a Q cut (default 12 Å⁻¹), then a logarithmic tail
    (default 160 points) to the beta grid's kinematic reach
    (`alpha_max = 4·beta_max/awr`). This layout resolves the upscatter windows
    that drive the thermal-energy cross section, which a beta-mirrored alpha
    grid under-samples.
...
recomputes the law at `|T|` (the LEAPR shared-DOS convention); no detail cards
follow it. Exception: the **first** temperature always supplies a detail
block, even if written negative (there is nothing yet to reuse; the law is
...
!!! warning "`inelastic_mode=1/2` decks omit the detail block"
    For `inelastic_mode=1/2`, the inelastic law comes from the phonopy model.
    Those decks supply **only** the temperature cards (Card 10); Cards 11–19
    are not read.
...
## End-to-end example: classic mode-0 deck (`iel=1`, graphite)

This is a complete classic deck: a built-in coherent-elastic material
...
result (the classic kernels reproduce published LEAPR tapes to ~1e-4). The
...
30 130/                               -- Card 4: mat=30, za=130 (graphite TSL ZA)
...
-400.0/         -- Card 10: negative T -> reuse the 296 K spectrum, recompute the law at 400 K
...
!!! note
    ...
    weight in the continuous spectrum, no translational or diffusion mode). The
    negative temperatures after the first reuse the 296 K detail block; this
    is how one phonon spectrum serves many temperatures.

## End-to-end example: generalized `iel=10`, `inelastic_mode=2` deck

This is a complete, runnable noncubic deck for hcp beryllium using the exact
coherent one-phonon path (`inelastic_mode=2`). It is the structure of the
committed mode-2 cross-validation template, with one addition: Card 4 sets
`iint=1` (lin-lin MF7/MT4 interpolation, the recommended pairing for sharp
coherent laws — see the `iint` tip above; for a production deck build the
grids with the `iint=1` [automatic grids](grids.md), whose high-β step is
capped for lin-lin). Substitute a real path for `PHONOPY_YAML_PATH` (the
`phonopy.yaml` plus its force-constants file must sit together). The
alpha/beta arrays are abridged with `…`.
...
' IRMA mode-2 exact one-phonon (nphon=1) cross-validation deck.   '/   -- comment -> MF1/MT451
' Coherent+incoherent n=1 S(a,b) for hcp beryllium.               '/
/                                                                 -- bare '/' ends comments
stop
```

!!! note "What is and isn't read in mode 2"
    Because `inelastic_mode=2`, the deck supplies only the temperature card
    (Card 10); no continuous-DOS / oscillator / Sköld detail block. `nspec=0`
    on Card 6b and Card 6e is absent. The inelastic law and the directional
    Debye-Waller factors come from the phonopy model named on Card 6f. To add
...
!!! note "Validation"
    ...
    Euphonic (an independent code, the same phonon model), coherent component
    against coherent component: the symmetric-law integrals agree to ratios of
    1.00001 (graphite) and 1.0002 (Be).
...
- For tapes destined for NJOY processing (THERMR/ACER), see
  [NJOY interoperability](njoy.md): coherent `inelastic_mode=2` laws can
