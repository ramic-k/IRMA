...
annotated example input files (a classic one and a generalized `iel=10`,
`inelastic_mode=2` one, with the arrays abridged) close the page.
...
are refused, and a scalar card never continues onto the next line.
...
Cryogenic tapes need `ilog=1` on Card 4. With `ilog=0` the symmetric thermal scattering law
...
The optional `iint` flag sets the interpolation law between stored points
(ENDF `INT`), while `ilog` sets how S is stored. It is written on both the α
(per-β `TAB1`) and β (`TAB2`) tables of MF7/MT4. The default, `iint=0`,
writes log-linear interpolation (`INT=4`), the classic, NJOY-faithful
choice. `iint=1` writes linear-linear (lin-lin,
...
THERMR (NJOY's thermal processing module) honors whichever flag is written.
Stock NJOY2016 THERMR ignores `INT` and interpolates ln S, so the lin-lin
benefit requires the patched THERMR described in
[NJOY interoperability](njoy.md). Use `iint=1` only with a Q/α grid
...
The phonopy-backed modes (`inelastic_mode=1` or `2`, set on
...
of Ramić et al., *NIM-A* 1027 (2022) 166227.
...
The principal nuclide may be split over several Card 6d entries. In every
mode, multiple Card 6d atom types that carry the principal nuclide (the
Card 4 `za`) are merged into a single group: their position lists are
combined, the coherent-elastic comb counts the principal once, and for
`inelastic_mode=1/2` the MT4 S(α,β) accumulates over every represented
...
```
'path/to/phonopy.yaml'  /                          Card 6f-1
mesh_nx  mesh_ny  mesh_nz  ncpu  use_born  /       Card 6f-2
'path/to/BORN'  /        (only if use_born = 1)    Card 6f-3
```
...
The card has one value and Card 6g has two or three, which is how the two
are told apart. Omitting it, or giving `0`, keeps IRMA's automatic mode
...
modes and the remaining spectrum is not renormalized: the evaluation
...
#### Card 6g: sampling controls (`inelastic_mode=1/2` only)
...
5 eV). The map from energy and Q to β and α contains kT, so the generator
anchors the grid to one reference temperature: 293.6 K for `lat=1` (a
stored value then means the same physical transfer at every temperature),
or the first temperature for `lat=0`. The alpha grid is linear in momentum
...
upscatter windows that drive the thermal-energy cross section, which an
alpha grid built by mirroring the beta grid through the recoil relation
under-samples.
...
(`iel=1`, graphite), the continuous-DOS inelastic path, and ten
temperatures (the first, then nine negative-temperature cards). It reproduces the published NJOY LEAPR crystalline-graphite
...
This is the structure of a mode-2 input file (arrays abridged) for hcp
beryllium using the exact coherent one-phonon treatment
(`inelastic_mode=2`). It follows the committed mode-2 cross-validation
template, and like it sets `nphon=1` with `auto_order` omitted (0): the
table carries only the one-phonon term, for cross-code validation, not an
evaluation. For an evaluation, use Card 6g `10000 1000 1` (auto-sized
multiphonon order), as `examples/tsl/graphite_mode2.input` does. The
template is followed with one addition: Card 4 sets
...
The mode-2 exact coherent one-phonon S(α,β) is cross-validated against
Euphonic; the [validation pages](validation/methodology.md) give the
results.

## See also

- [Automatic grids](grids.md) for the grid generators and their defaults.
