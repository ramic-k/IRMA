...
An IRMA deck is written in NJOY free-format and follows the LEAPR card
...
complete, runnable decks (a classic deck and a generalized `iel=10`,
`inelastic_mode=2` deck) close the page; copy and adapt them for your own
material.
...
header built from the deck's comment cards. The `irma` console script and the GUI
...
you can point IRMA at an existing NJOY deck without editing it.
...
| **4** | `mat za isabt ilog smin [iint]` | `0 0 0 0 1.0e-75 0` | ENDF `MAT` (`≥ 1`) and `za`; `isabt` (0/1) selects S(α,−β) storage; `ilog` (0/1) selects log storage; `smin` is the minimum stored S; optional `iint` (0/1) selects the MF7/MT4 interpolation scheme (see below, default 0). **For `iel=10`, `za` must encode the physical nuclide as `1000·Z + A`** so the principal scatterer can be matched to a Card 6d atom (`A = 0` names the natural element on both cards, e.g. `za = 6000` matching a Card 6d row with `A = 0`). |
...
byte-identical to pre-`iint` IRMA. `iint=1` writes linear-linear (lin-lin,
...
THERMR honors whichever flag is written. Use `iint=1` only with a Q/α grid
...
|-------|---------|
| `0` | None. |
| `1` | Vineyard. Cards 17–19 must be supplied, but, matching NJOY LEAPR, **no correction is applied**: with `ncold = 0` the resulting tape is identical to `nsk = 0` (with `ncold > 0`, the `S(κ)` table is used by the cold-hydrogen kernel exactly as it would be for any `nsk`). Use `nsk = 2` for a correction that modifies S(α,β). |
...
differing values are a Card 6d deck error. Non-principal types are never
merged.
...
How to produce the phonopy model this card names, from your own
force-constant calculation, is covered in
[Preparing a phonopy model](phonopy-input.md).
...
the tape's effective temperature). The auto-sizer raises the order to
...
dynamical-diffraction extinction, a property of the physical sample. The
card needs a coherent-elastic component: `elastic_mode=2` (MEF)
always has one; with `elastic_mode=1` (SEF) the deck must actually route
...
You do not have to build these grids by hand: the GUI and the bundled grid
generators can build them for you. The beta grid has three regions: a log
...
5 eV) and converted to dimensionless β at the deck's temperature. The alpha grid is linear in momentum
...
inelastic S(α,β) comes from the phonopy model, so those decks supply only
...
## End-to-end example: classic deck (`iel=1`, graphite)
...
the control cards above). For a production deck, build the grids with the
...
The mode-2 exact coherent one-phonon S(α,β) produced by this path has been
cross-validated against Euphonic (an independent code, the same phonon
model), coherent component against coherent component: the symmetric
S(α,β) integrals agree to ratios of 1.00001 (graphite) and 1.0002 (Be).
