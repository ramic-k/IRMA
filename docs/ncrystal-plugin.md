...
line (NCrystal core is scalar-MSD), and a **coherent one-phonon** inelastic law
(NCrystal's built-in inelastic is isotropic `vdos2sab`). IRMA is the single
producer of that physics (the **reference**), and NCrystal only samples the
result; there is no IRMA at NCrystal runtime.

The exporter is the `irma.ncrystal` module. It does **not** reimplement any
physics: the `S(α,β)` law it writes is exactly what the mode-2 engine
(`run_noncubic_standalone_sab`) returns, the same engine output that feeds an
NJOY tape. The exporter calls that engine once per principal scatterer,
converts conventions, and serializes.

## What it exports

For one config (one temperature) the exporter writes **one `.irmapack` per
principal scatterer**, plus a complete, loadable **`<material_id>.ncmat`** (the
phonopy structure + a `@CUSTOM_IRMA` section wiring the data files in):
...
- the **mode-2 inelastic** `S(α,β)` law for that principal species (coherent
...
- the **whole-crystal elastic line** (anisotropic-DW Bragg comb + the per-site
...
law and the anisotropic-DW structure factors at `material.temperature_K`, and
the C++ plugin only samples them. Re-run the exporter at each temperature you
need.

!!! note "Why per-temperature, not any-T"
    The mode-2 coherent line re-solves the dynamical matrix at every sampled
    `Q = G + q`. A true any-temperature C++ path would have to ship the force
    constants and port that dispersion re-solve: large, and unnecessary for the
    powder win. The data format is versioned so an any-T extension stays open.
...
section. Here is a complete, copy-pasteable graphite example:
...
The phonon model and per-species scattering data, identical to the
[spectra](spectra.md) and ENDF sides:
...
| `b_coh_fm` | Bound coherent scattering length (fm), drives the coherent Bragg comb, the coherent partition, and the mode-1/2 inelastic channel weights. **Required.** |
...
!!! note "List every species, even in BeO"
    `scatterers` must list **every** species the structure contains (each gets
    its own data file), not just one. A species in the phonopy cell with no
    matching scatterer entry is an error.
...
| `num_directions` | `10000` | Coherent powder-average directions for the one-phonon law. |
...
| `jobs` | `null` | Worker processes; `null`/omitted runs single-process. |
...
| `elastic` | `true` | Whether to attach the elastic line. When on, the data file carries the full physical elastic (coherent Bragg comb + incoherent Debye-Waller); isolate a channel at scatter time with NCrystal's `comp=coh_elas` / `comp=incoh_elas`. |
| `coherent_partition_mode` | `principal-xs-weighted` | How the engine splits the total coherent cross section across principal sites for the INELASTIC law (no double-count). |
| `incoherent_elastic_mode` | `isotropic` | Debye-Waller treatment of the pack's incoherent-elastic channel. `isotropic` collapses each site tensor to its trace/3 scalar (NCrystal's stock model). `directional` has the plugin sample the powder-averaged anisotropic `⟨exp(-Q² û·U·û)⟩` per site, larger at high Q for anisotropic crystals, and consistent with the directional multiphonon and the per-reflection coherent elastic. The ENDF tape path cannot represent this (its incoherent-elastic record stores a single scalar W′). |
...
| `n_lower`,`n_phonon`,`n_upper` | `50`,`300`,`80` | **Automatic** grid: β-grid point counts, log low-β tail, linear phonon region, log high-β tail. `n_upper=80` keeps the high-β tail fine enough to bake an accurate high-energy `S(α,β)` (≈20 suffices for thermal-only work; check convergence for your energy range and adjust up or down). |
...
Mirroring the ENDF-evaluation side, the grid is set one of two ways, and the
automatic grid is the *same converged grid the ENDF evaluator builds*
(`irma.core.grids.generate_beta_grid` / `generate_alpha_grid`, the same shared
code), so they never drift:
...
The exporter writes the **structure from the same phonopy cell the data was built
from**, so the per-site Debye-Waller tensors match the material's atom sites
exactly (the plugin pairs them by fractional position, tol 1e-6). A stock
stdlib NCMAT for the same compound usually has a different origin and will not
match. The placeholder `@DYNINFO` (a Debye VDOS back-derived from the phonopy
MSD) only lets NCrystal construct the crystal; the plugin overrides the inelastic
channel with the exported `S(α,β)` and, on the coherent-bearing data file, the
coherent/incoherent elastic with the anisotropic-DW line, so the placeholder
never reaches the cross section. The material temperature is set at load via
`;temp=...`, matching the export temperature — a mismatch is a hard error
(see [How NCrystal uses the data](#how-ncrystal-uses-the-data)).
...
| `elastic_u_frac_positions` | Each site's fractional position, the **key the plugin uses to pair tensors to the NCMAT atoms** (tol 1e-6). |
| `elastic_u_coherent_scatlen_sqrtbarn` | Per-site coherent scattering length, in √barn (parallel to the tensors). **Required whenever `elastic_u_tensors_a2` is present** — the loader rejects a pack that omits it — and every value must be finite. |
...
An inelastic-only data file (the non-elastic members of a polyatomic set) simply
...
4. **Channel routing (no double-count).** The plugin returns a *combined*
   process: its own model **plus** NCrystal's standard scatter with the channels
   the plugin supplies switched off, i.e.
...
temperature against each pack's bake temperature and **throws `BadInput`**
when they differ by more than a tight tolerance (~`1e-3 + 1e-5·T` K): it
samples the precomputed law and does not interpolate across temperatures, so
a mismatched request would silently be the wrong physics. Note that omitting
`;temp=` is not neutral — NCrystal then defaults to 293.15 K, which is a hard
error against any pack not baked at that temperature.
...
The **inelastic** law is cleanly per-species: each data file carries its own
species' mode-2 `S(α,β)`, and summing them sums the per-species inelastic
kernels with no ambiguity (the `principal-xs-weighted` coherent partition splits
the total coherent cross section exactly, no double-count: each pairwise
one-phonon interference term is shared between its two participants as
`w_p/(w_p+w_o)` by bound coherent cross section, so the per-species partials
sum back to the exact total for any number of site groups).
...
**single whole-crystal elastic line**, double-count-free and exact for the
total. (Graphite, with one data file, is the trivial case and is unaffected.)

## Conventions and provenance

A few convention bridges the exporter applies, worth knowing if you read a data
file or compare against an IRMA tape:
...
- **Negative-fringe clip.** Mode-2 coherent interference histograms can leave
  small negative cancellation bins in principal-partitioned laws; the exporter
  clips them to zero with a table-scale guard, the same projection IRMA's ENDF
  writer applies before NJOY sees the law.
...
at 296 K. A write-read-rewrite round trip of the material-data file is
byte-exact, and NCrystal returns identical component cross sections from
the original and round-tripped files, so serialization is lossless.

The consequential check compares the two programs that process the same
stored scattering law: the NCrystal plugin and corrected NJOY THERMR each
...
systematically lower: about 7.2% in the 10 μeV–25 meV integral, shrinking
to about 1.8% up to 0.5 eV. The origin is how each program turns the table
into a cross section: at low incident energy the kinematically allowed
...
!!! note "Export denser tables for transport"
    Grids denser than THERMR can process stably pose no difficulty for
    NCrystal, so an export intended for transport can simply use a denser
    table than the ENDF file; the exporter accepts explicit grids
    (see [the grid options](#two-ways-to-set-the-s-grid)).
...
- [NJOY interoperability](njoy.md): the other program that processes IRMA's
  mode-2 law, via an ENDF MF7 tape.
