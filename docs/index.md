...
writes it into MF7 thermal scattering data. It has four parts:
...
- **Generalized elastic and inelastic for any crystal.** Coherent-elastic Bragg
  ...
  built-in-material and cubic/isotropic restrictions. The mode-2 coherent
  one-phonon `S(α,β)` is cross-validated against Euphonic, an independent
  code run on the same phonon calculation, to shared-domain integral ratios
  of 1.00001 (graphite), 1.0002 (beryllium), and 0.9998 (BeO).
...
One principal scatterer, the atom species the evaluation is written for, is
evaluated per run. Polyatomic crystals (for example BeO) are handled by
keeping the full crystal in the phonon calculation and running each principal
species in a separate input file.
...
| Validated against NJOY | Reproduces published ENDF/B-VIII.1 reference tapes to 7e-5 (graphite, Fe, Al, H in CH2) and freshly generated NJOY2016.78 tapes exactly for liquid methane, ortho-/para-hydrogen, and two-pass beryllium oxide (covering the translational, discrete-oscillator, cold-hydrogen and mixed-moderator kernels); fast-CI minitapes pin cold deuterium and the Sköld correction against NJOY |
...
| Phonopy-backed inelastic (modes 1/2) | `inelastic_mode=1` (directional incoherent-approximation one-phonon + multiphonon) and `inelastic_mode=2` (**exact** coherent + incoherent one-phonon + incoherent multiphonon) driven by a phonopy calculation |
...
| NCrystal export | [`irma ncrystal`](ncrystal-plugin.md) writes the mode-1 or mode-2 S(α,β), the elastic data, and a record of the IRMA version and run settings as a per-temperature material data file for the companion NCrystal plugin, taking the same physics into Monte Carlo transport (McStas, OpenMC, ...) |
...
| `elastic_mode` | 6b | `1` SEF (single-channel elastic format; each species' tape holds one elastic component) · `2` MEF (mixed elastic, LTHR=3; needs downstream support) |
| `inelastic_mode` | 6b | `0` phonon expansion from the input file's DOS (isotropic Debye-Waller) · `1` directional incoherent approximation · `2` exact coherent one-phonon (+ incoherent-approximation multiphonon) |

Phonopy is needed only for inelastic modes 1 and 2: `inelastic_mode=1`
...
phonopy. Modes 1 and 2 also do not read the Card 11-19 detail block, and
they reject `ncold`, `nsk`, and a secondary scatterer.
...
modes 1/2: 10000 one-phonon powder directions, 1000 multiphonon
...
- **[Installation](installation.md)**: requirements (Python ≥ 3.11, numpy ≥ 2.0,
  ...
  `[phonopy]`/`[spectra]`/`[mlip]` extras, and verifying the install.
...
- **Tutorials**: [an ENDF evaluation from a phonopy
  calculation](tutorials/endf-evaluation.md), [a structure to a spectrum
  with an MLIP](tutorials/structure-to-spectrum.md), and [a transport
  material for NCrystal](tutorials/deck-to-transport.md).
- **[Preparing a phonopy calculation](phonopy-input.md)**: packaging your
  own force-constant calculation for IRMA.
...
- **[NCrystal data exporter](ncrystal-plugin.md)**: exporting the mode-1/2
  physics as material data files for Monte Carlo transport.
- **[The GUI](gui.md)**: interactive configuration, importing and exporting
  LEAPR-style input files, and the spectra and NCrystal configurations.
...
- **[Theory](theory.md)**: the physics behind each mode and the conventions
  used throughout the manual.
- **[Troubleshooting](troubleshooting.md)**: common input errors, warnings,
  and their fixes.
