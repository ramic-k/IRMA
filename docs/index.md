...
One phonon model feeds three outputs that usually require three separate tool
...
LEAPR-style card decks, reproduces the classic kernels, and extends them
...
serializes it into MF7 thermal scattering data. The code is built in layers
...
- **Classic LEAPR physics, reproduced.** The phonon-expansion kernels (continuous
  ...
  an independent code run on the same phonon model, to shared-domain integral
  ratios of 1.00001 (graphite) and 1.0002 (beryllium).
...
| Validated against NJOY | Reproduces published ENDF/B-VIII.1 reference tapes to 7e-5 or better (graphite, Fe, Al, polyethylene) and freshly generated NJOY2016 tapes exactly for liquid methane, ortho-/para-hydrogen, ortho-/para-deuterium, and beryllium oxide (covering the translational, discrete-oscillator, cold-hydrogen/deuterium, and Sköld kernels) |
...
| NCrystal export | [`irma ncrystal`](ncrystal-plugin.md) writes the mode-2 S(α,β), the elastic data, and provenance as a per-temperature material-data file for the companion NCrystal plugin, taking the same physics into Monte Carlo transport (McStas, OpenMC, ...) |
| Crystalline extinction | Opt-in [extinction correction](extinction.md) (Sabine and Becker-Coppens models, ported from CrysXT) for the coherent-elastic Bragg edges of sample-specific `iel=10` evaluations |
...
| `elastic_mode` | 6b | `1` SEF (single-channel elastic format, designated-coherent atom) · `2` MEF (mixed elastic, LTHR=3; needs downstream support) |
...
model (`phonopy.yaml` and force constants) supplied on Card 6f. The classic
paths (`iel=0`–`6`, and `iel=10` with `inelastic_mode=0`) do not need
...
`inelastic_mode=2` requests the exact one-phonon S(α,β) from a phonopy model:
...
so a faster look at `4000 200 1` is fine for exploration. The auto-sizer
raises the multiphonon order until the anisotropic Debye-Waller Poisson sum
