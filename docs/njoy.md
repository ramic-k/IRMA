...
IRMA writes standard ENDF-6 MF7 thermal scattering law tapes, so the
files it produces are meant to flow straight into NJOY's processing
chain. This page sketches how to push an IRMA MF7 tape through
RECONR / BROADR / THERMR / ACER, documents the two stock-NJOY THERMR
defects that matter for coherent `inelastic_mode = 2` tapes (and how to
patch them), and describes `thermr_mimic`, the offline validation tool
used to compare scattering laws without launching NJOY, together with
when a real THERMR run is unavoidable.
...
THERMR is the module that actually processes the scattering law, so it
is where the two patches below matter.

!!! note "Tape energies are in MeV"
    NJOY's tape34-style cross-section output reports incident neutron
    energies in **MeV**, not eV. Keep that in mind when you read back the
    inelastic cross section to compare against an IRMA-internal curve (the
    deck and IRMA's own grids work in eV / units of `kT`).
...
grid is spaced. This is a bug in stock `thermr.f90`, not in the IRMA
tape, and it is **not** cured by emitting a uniform energy grid.

!!! danger "Patch THERMR before processing coherent mode-2 tapes"
    Stock NJOY2016 THERMR can turn a correct IRMA coherent mode-2 law
    into ~`1e91`-barn garbage (or hang for hours) on the **shape** of the
    law, not its grid spacing. Apply the one-line two-axis guard fix
    below before trusting a mode-2 tape through NJOY. Most materials and
    all `inelastic_mode = 0/1` tapes are unaffected.
...
coherent mode-2 law has a `β = 0` row that decays in α (acoustic
...
!!! note "Grid spacing is a non-issue here"
    Earlier guidance suggested forcing a uniform ~1 meV beta grid for
    NJOY. The cliq blowup fires on the *shape* of a coherent law, so a
    uniform grid preserves the shape and does **not** prevent it. With
    the patched THERMR you can hand NJOY the standard IRMA automatic grid
    directly. (A uniform grid only ever sidestepped a separate, older
    log-tail stall; it was never the fix for the cliq garbage.)

Transfers that fall beyond the tabulated law are handled downstream by
THERMR's short-collision-time (SCT) extension, which is driven by the
...
that reconstructs the inelastic cross section; every law is treated as
log-lin there. Reported upstream as
[njoy/NJOY2016#410](https://github.com/njoy/NJOY2016/issues/410), with
the patch attached. For sharp coherent mode-2 laws written with IRMA's
`iint` option this matters: log-lin interpolation floors the deep
coherent near-zeros of the law and biases the cross section near the
inelastic minimum low (for graphite, lin-lin processing raises the
minimum by about 9% at 296 K and about 6% at 500 K; the schemes
...
Smooth incoherent laws (mode 0/1) have no such near-zeros, and log-lin
...
* **`iel = 5` (lead) coherent elastic.** NJOY ships `pb4 = 1.0` barn (a
  ...
  iel=5 tapes carry a coherent-elastic channel ~11.1x too small. IRMA
...
* **ln-S sentinel for zero-S points (ENDF writer).** NJOY's `endout`
  ...
  demonstrable NJOY bug. IRMA writes −999 at every temperature, keeping
  S ≈ 0 (safely below THERMR's `sabflg = −225` floor). Affects the
...
* **Print-flag-dependent SCT start index (phonon expansion).** The
  ...
  `if (iprint.ne.0)` in NJOY (`leapr.f90:566-571`) — NJOY's *physics*
...
* **Translational self-term clamp (`trans`).** NJOY clamps only the
  convolution part of the translational law (`leapr.f90:1041`) and
  ...
  self term landing in (1e-75, 1e-30) — reached only at high α, where
  the Debye-Waller weight `α·f0 ≳ 70` — is zeroed where NJOY keeps it.
...
* **Overflow guard in the discrete-oscillator Bessel factors
  ...
  coefficient of 0 puts NaN in the law. IRMA zeroes those terms; since
  ...
  expansion order ≳ 18).
* **SCT prefactor missing its square root (`sint`) — reproduced, not
  fixed.** The SCT tail evaluated when a discrete-oscillator or
  rotational shift pushes |β| past the tabulated range divides by
  `4π·wt·α·T̄`, where the Gaussian normalization — and every sibling
...
## Validating laws with `thermr_mimic`
...
compare two scattering laws (for example an IRMA mode-2 law against an
OCLIMAX-derived one) at the inelastic-cross-section level without
...
Use `thermr_mimic` when you want a fast, apples-to-apples comparison of
the *tabulated* law. Reach for **real THERMR** when you need the parts
of the processing that the mimic deliberately does not reproduce: the
short-collision-time extrapolation beyond the tabulated grid, the full
secondary energy–angle reconstruction, and the final ACE-format output.

!!! warning "Converted laws can diverge in real THERMR"
    Scattering laws converted from an external code (for example OCLIMAX
    `S(Q, E)` maps) can agree with the mimic yet **diverge in real
    THERMR** through the short-collision-time extrapolation. When you
    process such laws through NJOY, work from `thermr_mimic` for the
    grid-faithful comparison and from a **composite or uniformly
    resampled** tape for the actual THERMR run.
...
edges rather than a defect in the IRMA tape. This is an experiential
observation, not a guarantee: if you see them on a tape that also fails
downstream checks, investigate rather than assume they are benign.
