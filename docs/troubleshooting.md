...
IRMA evaluation (malformed input files, missing phonon data, incompatible mode
...
input file is rejected with the offending card, the expected and found values, and
...
## Input errors and what they mean

IRMA parses the input file in NJOY free-format style: values are separated by
...
IRMA raises an input error and the command-line tool prints it without a
traceback:
...
branch on the kind of failure: an input error (including an input file that ends
before a required card) exits with code `2`, and an otherwise meaningful
failure (a missing file, a phonopy calculation that will not load) exits with
code `3`.
...
|---|---|---|
| `input ended before this card` / `input ended after N of M expected values` | A card or array is short: the input file stops before all the values a count card promised | Supply the missing values, or correct the count on the preceding card |
...
The closing `/` of an array that spans
several lines (a long alpha grid, a phonon spectrum) goes on the
**last data line**, right after the final value, exactly as NJOY itself
writes input files. If you instead put a lone `/` on the *next* line by itself,
...
card far from the real mistake. When an input error points somewhere
...
The title on Card 2 is exempt from module-name detection, so a title
that happens to contain a module word will not truncate the input.

## phonopy not installed / force-constants resolution failures

The noncubic inelastic modes (`iel=10` with `inelastic_mode=1` or `2`)
need the optional `phonopy` extra. Neither the classic kernels (`iel=0`–`6`)
nor `iel=10` with `inelastic_mode=0` needs it.

```bash
pip install -e ".[phonopy]"   # from a source checkout; with pip: pip install "irma[phonopy]"
```
...
positions must match the phonopy calculation: for modes 1/2 the Card 6d fractional
...
The scan runs at every IRMA entry point that parses a phonopy.yaml: the ENDF
...
discussion in [MLIP phonon calculations](mlip.md) for what it does and does not
guarantee.

## "Modes 1/2 reject ncold / nsk / nss"

Modes 1 and 2 build the inelastic thermal scattering law
...
the classic options (`inelastic_mode=0` or `iel=0`–`6`).
...
it holding the *secondary* scatterer's values, so the whole elastic
section would describe the wrong species. An analytic secondary (`b7=1` free gas, `b7=2`
...
The second: SEF writes only the dominant elastic component, and two
configurations route that component to the incoherent-elastic form,
which never applies extinction: a single-atom principal with
`sigma_coh <= sigma_inc` (Card 6d), and a polyatomic principal that is
not the designated-coherent atom (the species
SEF selects to carry the coherent elastic: the one with the smallest
...
- **Set `auto_order=1`** in the third field of Card 6g. IRMA then raises
  the multiphonon order to converge the anisotropic Debye–Waller Poisson
  sum (the Poisson-weighted sum over phonon orders) at the grid's
  Q<sub>max</sub>. As a scale: graphite with
...
```
28 6012 0 1 /   <-- Card 4: mat za isabt ILOG=1 (smin defaulted)
```
...
This is a bug in stock `thermr.f90`, not in
...
The GUI (`irma-gui`, or `python -m irma --gui`) mirrors the input file format, with
...
- **"Preview Grid Sizes" reports the counts before you run.** On the Grids
  ...
  and `nbeta` are independent, so you can refine one axis without
  inflating the other.
...
optimum (jobs-vs-threads sweep on the graphite production NCrystal export:
...
line**; the file name is not taken from the input file:
...
- the full input file (or the smallest one that reproduces the problem),
- the exact console output, including any error message and line
  number,
- for modes 1/2, how the phonon calculation was produced (and whether
  `force_constants.hdf5`, `FORCE_CONSTANTS`, or `FORCE_SETS` was used).

A minimal reproducing input file is the single most useful thing you can
attach.
The committed example input files under `tests/` are good starting points to
adapt; see the [input file reference](input-reference.md) and
