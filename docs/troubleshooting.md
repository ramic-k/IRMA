...
!!! note "The CLI exits with distinct status codes"
    A deck error (including a deck that ends before a required card)
    exits with code `2`; an otherwise meaningful failure (a missing file, a
    phonopy model that will not load) exits with code `3`. Scripts can
    branch on these.
...
!!! warning "Where the `/` goes on a multi-line array"
    For arrays that span several lines (a long alpha grid, a phonon
    spectrum), the closing `/` must sit on the **last data line**, right
    after the final value, exactly as NJOY itself writes decks. If
    you instead put a lone `/` on the *next* line by itself, IRMA (like
    NJOY's list-directed read) rolls that stray `/` into the following
    scalar read, so that card silently takes its defaults and **every later
    card shifts by one**. The downstream symptom is a confusing error on a
    card far from the real mistake. When a deck error points somewhere
    unexpected, check the array terminators just above it.

!!! note "NJOY job streams are accepted"
    You can hand IRMA a full NJOY job stream; it locates the
    `leapr ... stop` block automatically and ignores the surrounding
    modules. The deck title on Card 2 is exempt from module-name detection,
    so a title that happens to contain a module word will not truncate the
    deck.
...
!!! warning "Common force-constant pitfalls"
    - **"Nothing found next to the yaml."** Put one of
      `force_constants.hdf5`, `FORCE_CONSTANTS`, or `FORCE_SETS` in the same
      directory as the `phonopy.yaml` you named on Card 6f.
    - **A `BORN` file in your working directory is ignored.** With
      `use_born=0`, IRMA disables phonopy's fallback of auto-reading a
      `BORN` file from the working directory. A non-analytical-term
      (LO–TO) correction is applied only via a top-level `nac:` block
      inside the named `phonopy.yaml`, or when you set `use_born=1` and give
      a readable BORN path; an unreadable BORN file is an error, not a
      silent skip.
    - **Atom positions must match the phonon model.** For modes 1/2 the
      Card 6d fractional positions must match the phonopy primitive cell.

!!! note "Unstable phonon models warn, they do not silently pass"
    If the phonon mesh contains imaginary modes (reported as negative
    frequencies), IRMA prints a warning naming the most-negative mode in
    meV. Those modes are excluded from the DOS-tensor grid and the mode
    sums, but the warning means the phonon model itself is unstable and
    should be reviewed before you trust the evaluation.
...
at parse time. A legitimate phonopy.yaml — written by phonopy itself — never
contains any of these, so the scan does not reject real models, and there is
deliberately no override switch.
...
note in [MLIP phonon models](mlip.md) for what it does and does not
guarantee.

## "Modes 1/2 reject ncold / nsk / nss"

The phonopy-backed MT4 path builds the inelastic law and the directional
Debye–Waller factors directly from the phonon mesh. It never constructs the
S(κ) tables that the cold-hydrogen and Sköld kernels consume, and there is
no noncubic counterpart to the secondary-scatterer pass. IRMA therefore
rejects those combinations **up front**, before the expensive phonon-mesh
load, instead of crashing mid-run:
...
**only** the temperature cards after the grids; the legacy scattering-law
detail block (continuous DOS, translational, oscillator, Sköld cards) is not
read at all. If you genuinely need cold hydrogen, Sköld, or a mixed
moderator, use the classic paths (`inelastic_mode=0` or `iel=0`–`6`).
...
The second: SEF writes only the dominant elastic channel, and that channel is
the incoherent one — which never applies extinction — when the single-atom
principal has `sigma_coh <= sigma_inc` (Card 6d), or when the polyatomic
principal is not the designated-coherent atom. To fix: use `elastic_mode=2`
(MEF) to keep a coherent channel, or remove the `extinction` card. See
[Crystalline extinction](extinction.md).
...
the tabulated law runs out of support: it goes identically zero before the
top of the grid. IRMA detects this and warns:
...
- **Set `auto_order=1`** in the 3rd field of Card 6g. The auto-sizer raises
  ...
  Q<sub>max</sub> ≈ 100 Å⁻¹ needs order ≈ 223; a fixed `nphon=100` truncates
  the high-Q rows, which is exactly what the warning is telling you.)
- **Raise Card 3 `nphon`** by hand until the law has genuine support over
  your beta grid.
...
!!! note "Transfers beyond the tabulated law are not lost"
    Energy transfers past the end of the tabulated law are reconstructed
    downstream by THERMR's short-collision-time extension, driven by the
    tape's effective temperature. The warning is about giving the *tabulated*
...
170–200 meV, for example) reads back as **zero**, the cause is the symmetric-law
storage underflowing, not a physics error.

With the default `ilog=0` (Card 4), the symmetric law is stored linearly as
`S·exp(−β/2)`. Because `β = E/kT` scales as `1/T`, at low temperature the
...
**0**, silently losing the structure (it is recovered on read as
`S = S_sym·exp(+β/2)`, so a stored zero stays zero).

IRMA now **warns** when this happens:
...
With `LLN=1` the law is stored as `ln(S)` (e.g. `ln(1e−100) = −230`, perfectly
representable), preserving the full dynamic range. Any reader must understand
`LLN=1`; THERMR does, and so do IRMA's own readers (`valplot.load_sym_sab`,
the validation harness, the neutron-scattering forward model). It is harmless at
room temperature, where `ilog=0` stays NJOY-byte-faithful.

!!! tip "Rule of thumb"
    For tapes below ~50–100 K, set `ilog=1`. The GUI's `ilog` control carries
    the same guidance, and the run log warns if you leave it at 0 and lose data.
    A self-contained numerical demonstration is in
...
!!! danger "Patch stock NJOY2016 THERMR for coherent mode-2 tapes"
    This is a bug in stock `thermr.f90` (a `cliq` liquid-extrapolation
    guard that tests decay along alpha but not beta), **not** in the IRMA
    tape, and it fires on the *shape* of a coherent law, so it is not
    fixed by any beta-grid choice. Apply the one-line two-axis guard
    patch before processing mode-2 tapes:
    ...
    `inelastic_mode = 0/1` tapes and most materials are unaffected.

For the mechanism, the symptom table, the full RECONR/BROADR/THERMR/ACER
recipe, and tape-energy units, see
...
- **The Phonon part warns when it is being ignored.** If `inelastic_mode=1/2`
  ...
  the inelastic law from the force constants.
...
intentionally pinned to one thread per worker; this is measured to be the
...
direction count (converged by ~50–100; the production default is 1000, the validation-campaign sampling). Soft-mode
materials auto-size the multiphonon Poisson order into the hundreds
(beta-quartz: 1084); the convolution ladder and the deposit sweep are
bounded per (direction, atom) row by the exact float64 underflow horizon
of the Poisson weights, so the deep orders cost only what the rows that
actually reach them require.

!!! note "Large grids make large tapes"
    ENDF tapes carry ~6-significant-figure values, and a big grid makes a
    big file: a 2000×5001 grid is on the order of 270 MB. Size the grid to
