...
This page gets you from a fresh install to a written ENDF tape (an ENDF-6
output file holding the thermal scattering law S(α,β), tabulated in the
dimensionless momentum transfer α and energy transfer β, with β = E/kT; the
[theory page](theory.md) defines the conventions) in about five minutes. You
will run a committed example input file through the command-line
...
A note on defaults: the GUI form, the inputs `irma mlip emit` writes,
and the spectra and NCrystal configurations default to the production
settings of the validation campaigns, so accepting them gives results of
the quality shown in the [validation record](validation/methodology.md). In
a hand-written input file, Card 6g's `auto_order` defaults to 0 (Card 3
`nphon` is used as given); set it to 1 for production.
...
(DOS) supplied in the input file and an isotropic Debye-Waller factor, the
same physics level as `iel=10` with `inelastic_mode=0`. It runs in seconds
and reproduces the
...
| `Found <N> Bragg edges below 5 eV` | The coherent-elastic phase. Printed when the principal scatterer has a built-in coherent-elastic treatment (`iel` = 1-6); `iel=10` prints `Generalized: found <N> Bragg edges below 5 eV`. |
...
Two echoed values need a note. `za=130` is the historical TSL convention for graphite
...
Before processing a coherent `inelastic_mode = 2` tape with NJOY, note that
stock NJOY2016 THERMR has a bug in `cliq` that produces cross sections near
1e91 b above about 0.27 eV for a correct tape (graphite and similar), or
stalls for hours. The bug fires on the *shape* of the
...
level as `inelastic_mode=0`: the phonon expansion in the incoherent
approximation with an isotropic Debye-Waller factor, built from a scalar
phonon DOS you supply in the input file. No phonopy is needed.
...
and the phonopy-backed `inelastic_mode=1/2` workflows are documented card by card in
...
coherent-elastic treatment: `0` = none, `1`-`6` = the built-in
materials (graphite, Be, BeO, Al, Pb, Fe), and `10` = generalized Bragg edges
from your own crystal structure. For a new material, `iel=10` is
recommended; it also makes the anisotropic Debye-Waller
...
auto-grid flag. Automatic grids are built in the GUI (the **Grids** part of
the ENDF Evaluation tab) or from Python via the `irma.core.grids` functions;
either way the result is an explicit grid that lands on Cards 7–9 of the
input file. The [Automatic grids](grids.md) page explains the controls
...
windows that an alpha grid built by mirroring the beta grid through the
recoil relation under-samples.
...
data, run the calculation with a progress display, and export the form as an
input file (**File > Export Input File**). Existing `.input` / `.leapr` input
files can be imported via **File > Import Input File** to populate every
field. The GUI is covered in
...
ratio, b_coh [fm], sigma_inc [b]; the CLI does not look these up, so pass
all five, while the GUI fills them from IRMA's nuclear-data table):
...
- **[Input file reference](input-reference.md)**: the authoritative
  card-by-card spec, including `iel=10` and inelastic modes 1/2.
