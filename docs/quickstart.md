...
This is a classic crystalline-graphite evaluation (deck-supplied DOS, isotropic
Debye-Waller, the same physics level as `iel=10` with `inelastic_mode=0`); it
runs in seconds and reproduces the published ENDF/B-VIII.1 graphite tape to
3.4×10⁻⁵ (the graphite entry of the [reference set](validation/methodology.md)).

!!! note "Console script and command forms"
    If you installed IRMA with `pip`, the bare `irma` console script is
    equivalent to `python -m irma`: `irma input.input out.endf`. The
    two-positional deck form also has explicit aliases that do exactly the same
    thing (`irma evaluate input.input out.endf` and `irma run input.input
...
output looks like this (the middle temperatures are elided here):
...
| Echoed control cards (`ntempr`, `mat`, `awr`, `nalpha`, ...) | IRMA reads and prints back the values it parsed from Cards 3-7. Use these to verify the parser saw what you intended, wrong `iel` or `nbeta` shows up here. |
...
| `Writing ENDF output...` / `ENDF output written to <path>` | The S(α,β) law and elastic data are assembled into MF7 and serialized to the output path. |
...
!!! danger "Coherent mode-2 tapes bound for NJOY need a THERMR patch"
    Stock NJOY2016 THERMR has a `cliq` bug that turns a correct coherent
    `inelastic_mode = 2` tape (graphite and similar) into ~`1e91`-barn
    garbage above ~`0.27 eV`, or stalls for hours. It fires on the law's
    *shape*, not its grid, so no beta-grid choice avoids it. Apply the
...
card is terminated by `/`. The simplest useful deck is a classic
`inelastic_mode=0` evaluation: an isotropic Debye-Waller treatment plus a
cubic phonon expansion built from a scalar density of states you supply on the
...
| 11-16 | `delta ni`, `rho`, `twt c tbeta`, `nd`, ... | The scattering-law detail block (DOS, translational, oscillators). |

!!! note "This is the orientation, not the full spec"
    Field meanings, defaults, valid ranges, the generalized-elastic cards
    (`iel=10`, Cards 6b-6g), and the noncubic `inelastic_mode=1/2` workflows
    are documented card-by-card in the
    [Input cards reference](input-reference.md). Reach for it whenever a field
    above is unclear.

!!! warning "Choosing the elastic and inelastic mode"
    Card 5 `iel` selects the coherent-elastic treatment: `0` = none,
    `1`-`6` = the legacy built-in materials (graphite, Be, BeO, Al, Pb, Fe),
    and `10` = generalized Bragg edges from your own crystal structure. For a
    new material on a noncubic crystal, `iel=10` is the recommended path; it
    unlocks the anisotropic Debye-Waller and phonopy-backed
    `inelastic_mode=1/2` workflows. The example above uses the built-in
    `iel=1` graphite tables, which keeps the deck self-contained.
...
INS instrument measures. One command produces a VISION-resolved spectrum from
a phonopy model:
