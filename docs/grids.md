...
| Upper tail | Logarithmic for log-lin (`iint=0`) input files; step-capped below the recoil ridge for lin-lin (`iint=1`) input files (see [the lin-lin upper tail](#the-upper-tail-on-lin-lin-int2-grids)) | **Beta N upper (log)** and **Beta max** | Multiphonon contributions and the smooth exponential high-energy tail |
