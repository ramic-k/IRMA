...
| `n_upper` | Beta N upper (log) | 80 | High-$\beta$ tail density (GUI default; the `generate_beta_grid` function default is 20 for log-lin grids, byte-stable, and 80 for lin-lin). On a lin-lin (`iint=1`) grid the tail gains extra step-capped points below the recoil ridge, so the final tail count exceeds `n_upper` (see below) |
...
The GUI defaults `n_upper` to 80; the `generate_beta_grid` log-lin default is 20 so existing log-lin grids stay byte-identical.
